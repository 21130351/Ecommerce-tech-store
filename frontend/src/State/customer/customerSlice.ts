...
//         } catch (error: any) {
//             const errorMessage = error.response?.data?.message || error.message || "Failed to fetch Home page";
...
export const createHomeCategories = createAsyncThunk<HomeData, HomeCategory[]>(
    ...
    async (homeCategories, {rejectWithValue}) => {
        try {
            ...
            const errorMessage = error.response?.data?.message || error.message || 'Failed to create home category';
            console.log("error ",errorMessage,error)
            return rejectWithValue(errorMessage);
